...
$plc = array(
	'fermi'=>'<label>
  <input type="radio" name="plc" value="L"<!--L-->> L
</label>
<label>
  <input type="radio" name="plc" value="H"<!--H-->> U
</label>
',
	'elettra'=>'<label>
  <input type="radio" name="plc" value="sr"<!--sr-->> sr
</label>
<label>
  <input type="radio" name="plc" value="bo"<!--bo-->> bo
</label>');
	...
	$db = mysqli_connect(HOST, "eacro", "", "elettraaccesscontrol");
	// ----------------------------------------------------------------
	...
	function emit_stat_bo()
	{
		...
		$db = mysqli_connect(HOST, "eacro", "", "elettraaccesscontrol");
		// mysqli_select_db("elettraaccesscontrol", $db);
		...
		$db = mysqli_connect(HOST, "BACro", "Bacolo", "BoostAccessCtrl");
		// mysqli_select_db("BoostAccessCtrl", $db);
		if (isset($_REQUEST["query"])) {
			debug($_REQUEST["query"]);
			$res = mysqli_query($_REQUEST["query"]);
		...
		$res = mysqli_query("SELECT byte, name FROM access_desc_string ORDER BY byte");
		while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
		...
		if (strlen($filter)) {
			...
			$res = mysqli_query("SELECT * FROM state_desc_bit WHERE name LIKE '%$filter%' OR descr LIKE '%$filter%'");
			while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
				$offsetArray[] = $r["byte"];
				$bitArray[] = $r["bit"];
				$ids[] = $r["id"];
			...
			$res = mysqli_query("SELECT * FROM state_desc_bit WHERE id IN ({$_REQUEST["names"]})");
			while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
				$offsetArray[] = $r["byte"];
				$bitArray[] = $r["bit"];
			}
		...
		$res = mysqli_query("SELECT id, name FROM state_desc_bit ORDER BY name");
		while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
		...
		$res = mysqli_query($statquery);
		// echo "$statquery<br>";
		if (mysqli_errno()) {
			echo "Error ".mysqli_errno().": ".mysqli_error()."<p>\n";
		}
		$oldval = "";
		$first = true;
		while ($d = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
			...
			if (isset($_REQUEST["var"])) {
				...
				foreach ($diff as $i => $val) {
					...
					if ($i/2 > 327) {
						...
						$byteq = mysqli_query("SELECT name FROM state_desc_byte WHERE byte=".floor($i/2));
						$byte = mysqli_fetch_array($byteq, MYSQLI_ASSOC);
						$byteqdesc = mysqli_query("SELECT message AS descr FROM msg_display WHERE code=$newVal");
						$bytedesc = mysqli_fetch_array($byteqdesc, MYSQLI_ASSOC);
					...
					for ($j=1,$k=0; $k<=3; $j*=2, $k++) {
						if (($oldVal & $j) !== ($newVal & $j)) {
							$byteq = mysqli_query("SELECT name, descr FROM state_desc_bit WHERE byte=".floor($i/2)." AND bit=".($k+4-($i%2)*4));
							$byte = mysqli_fetch_array($byteq, MYSQLI_ASSOC);
							$address = "byte: ".floor($i/2).", bit: ".($k+4-($i%2)*4);
							$label = (isset($byte["name"]) and strlen($byte["name"]))? "<span title='$address' alt='$address'>{$byte["name"]}</span>": $address;
			...
			if ($first) {
				...
				if (isset($_REQUEST["names"]) and strlen($_REQUEST["names"])) {
					$r = mysqli_query("SELECT name, descr FROM state_desc_bit WHERE id IN ({$_REQUEST["names"]})");
					while ($c = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
						$label = $c["name"];
	...
	function emit_presence_bo() {
		...
		if (!empty($res)) while ($d = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
	...
	function emit_stat_bo_new() {
		...
		while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
			...
			if (!isset($linac_db51_old[$r["byte_number"]])) {
				...
				$old_res = mysqli_query($query);
				if ($debug) {echo "<!--\n"; debug($query); echo "\n-->\n";}
				$old_r = mysqli_fetch_array($old_res, MYSQLI_ASSOC);
	...
	function emit_stat_sr()
	{
		...
		if (!isset($_REQUEST["startdate"])) return;
		$stopdate = (isset($_REQUEST["stopdate"]) and strlen($_REQUEST["stopdate"]))? " AND plc_time<=UNIX_TIMESTAMP('{$_REQUEST["stopdate"]}')": "";
		$res = mysqli_query("SELECT byte_number, bit_number, name, comment FROM sr_db51_descr ORDER BY byte_number, bit_number");
		while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
			$var_names[$r["byte_number"]][$r["bit_number"]] = $r["name"];
			$var_desc[$r["byte_number"]][$r["bit_number"]] = $r["comment"];
			$filter_select .= "<option value='{$r["name"]}'>{$r["name"]} - L{$r["byte_number"]}.{$r["bit_number"]}</option>\n";
	...
	function eval_filter() {
		...
		// mysqli_select_db("elettraaccesscontrol", $db);
		// $res = mysqli_query("SELECT byte_number, bit_number, name, comment FROM sr_db51_descr ORDER BY byte_number, bit_number");
		$res = mysqli_query("SELECT byte_number, bit_number, name, comment FROM sr_db51_descr ORDER BY name");
		while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
		...
		// mysqli_select_db("BoostAccessCtrl", $db);
		// $res = mysqli_query("SELECT byte, name FROM access_desc_string ORDER BY byte");
		$res = mysqli_query("SELECT * FROM state_desc_bit ORDER BY name");
		while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
			$access_desc_string[$r['byte'] / 16 -1] = $r['name'];
	...
	function emit_data($data1,$data2) {
		...
		$template = strtr(file_get_contents('./pss_header.html'), array('<!--machineUC-->'=>'ELETTRA', '<!--plc-->'=>$plc['elettra']));
	...
	// ----------------------------------------------------------------
	// check presence of people
	function emit_presencelh() {
		global $debug, $script, $statquery, $admin, $db;
		$startdate = isset($_REQUEST["startdate"])? parse_time($_REQUEST["startdate"]): date("Y-m-d H:i:s", time()-600);
		$stopdate = (isset($_REQUEST["stopdate"]) and strlen($_REQUEST["stopdate"]))? " AND plc_time<=UNIX_TIMESTAMP('".parse_time($_REQUEST["stopdate"])."')": "";
		// $db = mysqli_connect(HOST, "eacro", "");
		// mysqli_select_db("elettraaccesscontrol", $db);
		$yy = substr($startdate, 0, 4);
  ...
  // ----------------------------------------------------------------
  // check access as administrator
  function emit_statlh()
  {
    global $debug, $script, $statquery, $admin, $filter_select, $db;
    // debug($_REQUEST);
    $id = 0;
    $ids = array();
    ...
    $filter_select = "<select name='filter_name'>\n<option value=''> </option>\n"; 
    $res = mysqli_query($db, "SELECT byte_number, bit_number, name, comment FROM linac_db51_descr ORDER BY name");
    // $res = mysqli_query($db, "SELECT byte_number, bit_number, name, comment FROM linac_db51_descr ORDER BY byte_number, bit_number");
    while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
    ...
    // $res = mysqli_query($db, "SELECT byte_number, bit_number, name, comment FROM undulator_db51_descr ORDER BY byte_number, bit_number");
    $res = mysqli_query($db, "SELECT byte_number, bit_number, name, comment FROM undulator_db51_descr ORDER BY name");
    while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
    ...
    //$res = mysqli_query($db, "SELECT 'linac' AS plc, CONCAT(FROM_UNIXTIME(plc_time),SUBSTR(MOD(plc_time,1),2)) AS t, plc_time, byte_number, value FROM linac_db51_$year, linac_time_$year WHERE time_id=id AND plc_time>UNIX_TIMESTAMP('$startdate')");
    ...
    if ($_REQUEST['plc']=='all') $res = mysqli_query($db, "$linac_query\nUNION\n$undulator_query\nORDER BY t, byte_number");
    if ($_REQUEST['plc']=='L') $res = mysqli_query($db, "$linac_query ORDER BY t, byte_number");
    if ($_REQUEST['plc']=='H') $res = mysqli_query($db, "$undulator_query ORDER BY t, byte_number");
		...
		while ($r = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
			...
			if ($r['plc']=='linac') {
				if (!isset($linac_db51_old[$r["byte_number"]])) {
					$old_res = mysqli_query($db, "SELECT value FROM linac_db51_$year, linac_time_$year WHERE time_id=id AND plc_time<{$r['t']} AND byte_number={$r["byte_number"]} ORDER BY plc_time DESC LIMIT 1");
					$old_r = mysqli_fetch_array($old_res, MYSQLI_ASSOC);
					$linac_db51_old[$r["byte_number"]] = isset($old_r["value"])? $old_r["value"]: 0;
				}
				for ($i=0; $i<8; $i++) {
				...
				if (!isset($undulator_db51_old[$r["byte_number"]])) {
					$old_res = mysqli_query($db, "SELECT value FROM undulator_db51_$year, undulator_time_$year WHERE time_id=id AND plc_time<{$r['t']} AND byte_number={$r["byte_number"]} ORDER BY plc_time DESC LIMIT 1");
					$old_r = mysqli_fetch_array($old_res, MYSQLI_ASSOC);
					// echo "{$old_r["value"]}<br>";
...
*/
	...
	// ----------------------------------------------------------------
	// display data in HTML
	function emit_datalh($data1) {
		global $debug, $script, $filter_select;
		// if (isset($_REQUEST['debug'])) {debug($data1); exit();}
		if (!empty($_REQUEST['filter_name'])) {$_REQUEST['filter'] = $_REQUEST['filter_name'];}
		if (isset($_REQUEST['export']) and ($_REQUEST['export']=='csv')) {emit_csvlh($data1);}
		$template = strtr(file_get_contents('./pss_header.html'), array('<!--machineUC-->'=>'FERMI', '<!--plc-->'=>$plc['fermi']));
		$replace = array("<!--startdate-->"=>$_REQUEST["startdate"],"<!--stopdate-->"=>$_REQUEST["stopdate"]);
		...
		echo "</table>\n";
		if (isset($_REQUEST['debug'])) echo "emit_presencelh()<br>\n";
		emit_presencelh();
		echo "</div>\n</div>\n";
		readfile('./footer.html');
	}

$machine = isset($_REQUEST['machine'])? $_REQUEST['machine']: (strpos($_SERVER["HTTP_HOST"],'fcs')!==false? 'fermi': 'elettra');
  
if ($machine=='fermi') {
	if (!isset($_REQUEST['plc'])) $_REQUEST['plc']='all';
	emit_datalh(emit_statlh());
	exit();
}
