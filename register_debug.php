...
<html lang="it">
  <head>
	...
	function delete_backoffice() {
		...
		header("location: ./register_debug.php?machine=$machine&backoffice");
		exit();
	}
	...
	function update_backoffice() {
		...
		$replace['<!--dosimeter_value-->'] = $d['dosimeter_number']>0? $d['dosimeter_value']: '';
		...
		$replace["register.php"] = "register_debug.php";
	...
	<!-- Custom styles for this template -->
	<link href="./lib/bootstrap/css/theme.css" rel="stylesheet">
	</head><body>&nbsp;<h1>Attenzione</h1>&nbspSi &egrave; verificato un errore, se dovesse permanere &egrave; obbligatorio usare il registro cartaceo<br><br>&nbsp<a href="./register_debug.php">continua</a><br><br><br><br>');
		}
	}
	...
	if (isset($_REQUEST['name_0'])) {
		...
		header("location: ./register_debug.php?machine=$machine".(strlen($err_msg)? "&msg=$err_msg": ''));
		exit();
	}
	...
	if (isset($_REQUEST['edit'])) {
		$template = explode('<!--item-->', strtr(file_get_contents("register_$machine.html"),array('register.js'=>'register_debug.js','register.php'=>'register_debug.php')));
	...
	if (isset($_REQUEST['del'])) {
		...
		$res = $sql->sql_query($logquery);
		if (!$res) {printf("$logquery;<br>\nErrormessage: %s\n", $sql->sql_error());exit();}
		header('Location: ./register_debug.php');
	...
	if (isset($_REQUEST['enter'])) {
		...
		header("location: ./register_debug.php?machine=$machine");
		exit();
	}
